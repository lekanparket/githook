...
class GitSecureFileReader:
    ...
    def detect_secret_keys(self):
        ...
        for line in lines:
            count += 1
            split = line.split('=')[1:]
            line = ''.join(split).replace('"','').replace("'",'').replace(' ', '')#line.split('=')[1:].replace('"','').replace("'",'"').replace(' ', '')
            ...
            for i, values in enumerate(secret_key_vals):
                ...
                if vv:
                    ...
                    print(bcolors.WARNING  + f"Warning: Line {count} has a {secret_key_keys[i]} " + bcolors.BOLD + 
                        bcolors.FAIL + f'{line}', bcolors.ENDC + bcolors.WARNING + "please have a look." + bcolors.ENDC)
                    break
